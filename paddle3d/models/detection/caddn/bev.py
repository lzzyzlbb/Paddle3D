...
class BEV(nn.Layer):
    def __init__(self, layer_nums, layer_strides, num_filters, 
                upsample_strides, num_upsample_filters, input_channels):
        super().__init__()
        ...
        for idx in range(num_levels):
            ...
            self.blocks.add_sublayer("level_"+str(idx), nn.Sequential(*cur_layers))
            if len(upsample_strides) > 0:
                stride = upsample_strides[idx]
                if stride >= 1:
                    self.deblocks.add_sublayer("level_"+str(idx), nn.Sequential(
                        nn.Conv2DTranspose(
                            num_filters[idx], num_upsample_filters[idx],
                            upsample_strides[idx],
                            stride=upsample_strides[idx], bias_attr=False
                        ),
                        nn.BatchNorm2D(num_upsample_filters[idx], epsilon=1e-3, momentum=0.99),
                        nn.ReLU()
                    ))
                else:
        ...
        if len(upsample_strides) > num_levels:
            self.deblocks.add_sublayer("upsample", nn.Sequential(
