    ...
    def generate_anchors(self, grid_sizes):
        ...
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
            ...
            x_shifts = paddle.arange(
                self.anchor_range[0] + x_offset, self.anchor_range[3] + 1e-5, step=x_stride, dtype='float32',
            )
            y_shifts = paddle.arange(
                self.anchor_range[1] + y_offset, self.anchor_range[4] + 1e-5, step=y_stride, dtype='float32',
            )
            ...
            anchors = anchors.unsqueeze([3]).tile([1, 1, 1, paddle.shape(anchor_size)[0], 1])
            anchor_size = anchor_size.reshape([1, 1, 1, -1, 3]).tile([*paddle.shape(anchors)[0:3], 1, 1])
            anchors = paddle.concat((anchors, anchor_size), axis=-1)
            anchors = anchors.unsqueeze([4]).tile([1, 1, 1, 1, num_anchor_rotation, 1])
            anchor_rotation = anchor_rotation.reshape([1, 1, 1, 1, -1, 1]).tile([*paddle.shape(anchors)[0:3], num_anchor_size, 1, 1])
