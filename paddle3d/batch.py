...
def padding_sample(samples: List[Sample]):
...
def collate_fn(batch: List):
    ...
    if isinstance(sample, np.ndarray):
        ...
        valid_keys = [key for key, value in sample.items() if value is not None]
        padding_sample(batch)

        return {key: collate_fn([d[key] for d in batch]) for key in valid_keys}
