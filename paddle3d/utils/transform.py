...
def convert_points_to_homogeneous(points):
    ...
    if len(list(paddle.shape(points))) == 3:
        ...
        data_format="NCDHW"
    return paddle.nn.functional.pad(points, [0, 1], "constant", 1.0, data_format=data_format)
...
def transform_points_3d(trans_01, points_1):
    ...
    trans_01 = trans_01.tile([paddle.shape(points_1)[0] // paddle.shape(trans_01)[0], 1, 1])
    # to homogeneous
    
    points_1_h = convert_points_to_homogeneous(points_1)  # BxNxD+1
    
    # transform coordinates
    points_0_h = paddle.bmm(points_1_h, trans_01.transpose([0, 2, 1]))
